...
from ...core.database.models import Task, Flow
...
def list(flow_id: Optional[str], status: Optional[str], limit: int):
        ...
        async with get_session() as session:
            ...
            for task in tasks:
                rows.append([
                    ...
                    task.completed_at.strftime('%Y-%m-%d %H:%M:%S') if task.completed_at else 'N/A',
                    task.error if task.error else ''
                ])
            
            # Print table
            headers = ['ID', 'Flow', 'Status', 'Created', 'Completed', 'Error']
            click.echo("\nFlow Tasks:")
        ...
        async with get_session() as session:
            ...
            if task.logs:
                ...
                for log in task.logs:
                    click.echo(f"[{log.created_at.strftime('%Y-%m-%d %H:%M:%S')}] {log.message}")
