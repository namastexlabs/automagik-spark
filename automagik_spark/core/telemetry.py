...
class TelemetryClient:
    ...
    def _is_telemetry_enabled(self) -> bool:
        ...
        if os.getenv("AUTOMAGIK_SPARK_DISABLE_TELEMETRY", "false").lower() in ("true", "1", "yes", "on"):
            return False
            ...
            "resourceSpans": [{
                "resource": {
                    "attributes": [
                        ...
                        {"key": "telemetry.sdk.name", "value": {"stringValue": "automagik-spark"}},
    ...
    def track_installation(self, install_type: str = "pip") -> None:
        ...
        self._send_event("installation", {
            "install_type": install_type,
            "first_run": True
        })
    
    def track_command(self, command: str, duration: Optional[float] = None, 
    ...
    def track_workflow_execution(self, workflow_type: str, duration: Optional[float] = None,
                               success: bool = True, error: Optional[str] = None) -> None:
...
def track_command(command: str, duration: Optional[float] = None, 
                 success: bool = True, error: Optional[str] = None) -> None:
    """Track CLI command usage."""
...
def track_workflow_execution(workflow_type: str, duration: Optional[float] = None,
                           success: bool = True, error: Optional[str] = None) -> None:
    """Track workflow execution."""
